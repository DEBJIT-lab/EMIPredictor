...
export const Route = createRootRouteWithContext<{ queryClient: QueryClient }>()({
  head: () => ({
    meta: [
      ...
      { title: "EMI Risk Intelligence Platform" },
      {
        name: "description",
        content:
          "Machine-learning EMI eligibility and affordability assessment with MLflow-tracked models.",
      },
    ...
    <header className="sticky top-0 z-40 border-b border-border/70 bg-background/85 backdrop-blur">
      <div className="mx-auto flex max-w-7xl flex-wrap items-center gap-x-6 gap-y-3 px-5 py-3">
        <Link to="/" className="flex items-center gap-2">
          ...
          <span className="font-display text-base font-semibold">EMI Risk Intelligence</span>
        </Link>
    ...
    <QueryClientProvider client={queryClient}>
      <div className="min-h-screen">
        ...
        <footer className="mt-16 border-t border-border/70 py-8 text-center text-xs text-muted-foreground">
          Models tracked in MLflow · XGBoost classifier & regressor registered v1 · 400,000 record
          training corpus
        </footer>
      </div>
      <Toaster />
